                            ...
                            <tr>
                                ...
                                <td>
                                    @php
                                        ...
                                        // fallback ke factory jika file tidak ada di path upload
                                        if (
                                            !file_exists(public_path($imagePath)) &&
                                            in_array($imagePath, $factoryImages)
                                        ) {
                                    ...
                                    @if ($imagePath && file_exists(public_path($imagePath)))
                                        <img src="{{ asset($imagePath) }}" alt="{{ $product->name }}" width="50"
            ...
            <div class="mt-4">
                <nav aria-label="Page navigation">
                    <ul class="pagination justify-content-center">
                        ...
                        @else
                            <li class="page-item"><a class="page-link btn-warning"
                                    href="{{ $products->previousPageUrl() }}">← Sebelumnya</a></li>
                        @endif

                        {{-- Pagination Elements --}}
                        @foreach ($products->getUrlRange(1, $products->lastPage()) as $page => $url)
                            @if ($page == $products->currentPage())
                                <li class="page-item active"><span class="page-link btn-warning">{{ $page }}</span>
                                </li>
                            @else
                                <li class="page-item"><a class="page-link btn-warning"
                                        href="{{ $url }}">{{ $page }}</a></li>
                        ...
                        @if ($products->hasMorePages())
                            <li class="page-item"><a class="page-link btn-warning"
                                    href="{{ $products->nextPageUrl() }}">Selanjutnya →</a></li>
                        @else
